...
public class SubTaskView_GroupingTask extends SubTaskView{
	...
	public String getCorrectedHTML( ViewContext context, int relativeTaskNumber ){
		...
		if(System.getProperty("os.name").toLowerCase().indexOf("win") >= 0) // windows = extrawurst
			name = "\\opt\\apache-tomcat-7.0.29\\webapps\\taskmodel-core-view\\pdfimgexport\\Grouping"+relativeTaskNumber+".".concat(typ);
		else name = "/opt/apache-tomcat-7.0.29/webapps/taskmodel-core-view/pdfimgexport/Grouping"+relativeTaskNumber+".".concat(typ);
		nameWebapp = "/taskmodel-core-view/pdfimgexport/Grouping"+relativeTaskNumber+".".concat(typ);
		...
		String imgTag = "<img src=\""+nameWebapp+"\" alt=\"groupingImg\" width=\""+width+"\" height=\""+height+"\">";		
		return imgTag;
	}
