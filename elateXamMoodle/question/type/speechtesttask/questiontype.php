...
/**
 * Question type class for the speechtesttask question type.
 *
 * @package    qtype
...
class qtype_speechtesttask extends elate_addon_questiontype_base {

	public function extra_question_fields() {
		return array('question_speechtesttask', 'correctorfeedback', 'memento');
	}
	...
	public function save_question_options($question) {
		if(isset($question->memento)) {
			...
			//Add stuff for additional options in speechtesttask
			$Count = $dom->createElement('playCount', $question->{"playCount"});
			$Filepath = $dom->createElement('filePathMP3', $question->{"filePathMP3"});
	...
	public function get_question_options($question) {
		if(parent::get_question_options($question)) {
			...
			//$XPath = new DOMXPath($dom);
			// "unpack" Memento, see qtype_speechtesttask::save_question_options()
			//$entries = $XPath->query("//question/*");
			
			//Get additional stuff from memento for speechtest
			$Count = $dom->getElementsByTagName('playCount');
			$Filepath = $dom->getElementsByTagName('filePathMP3');
...
foreach ($Count as $iterate) {
...
}foreach ($Filepath as $iterate) {
       $question->{"filePathMP3"} = $iterate -> nodeValue;
}foreach ($Delay as $iterate) {
       $question->{"delayForAudioInSeconds"} = $iterate -> nodeValue;
}foreach ($Maxtime as $iterate) {
       ...
       $question->{"isRestricted"} = $iterate -> nodeValue;
			...
			//$question->{"filePathMP3"} = $Filepath->nodeValue;
			//$question->{"delayForAudioInSeconds"} = $Delay->childNode->nodeValue;
			//$question->{"maximumTimeForTask"} = $Maxtime->childNode->nodeValue;
			//$question->{"timetostartreplay"} = $Timetostartreplay->nodeValue;			
	 ...
	 * @see qtype_speechtesttask::convert_images_to_base64()
	 * @see qtype_speechtesttask::convert_images_from_base64()
	 *
