...
/**
 * Defines the editing form for the speechtesttask question type.
 *
 * @package    qtype
...
class qtype_speechtesttask_edit_form extends elate_question_edit_form {
	...
	protected function definition_inner($mform) {
		...
		global $PAGE;
		$PAGE->requires->js("/question/type/speechtesttask/module.js");
	}
	...
	protected function add_question_fields() {
		...
		//Additional options for speechtest in addition to r
		$mform->addElement('text', 'playCount', get_string('playCount', "qtype_speechtesttask"), 'maxlength="5" size="5" ');
		$mform->addElement('text', 'filePathMP3', get_string('filePathMP3', "qtype_speechtesttask"), 'maxlength="200" size="50" ');
		...
		$mform->setDefault("isRestricted", 1);
		...
		$mform->addElement('submit', 'addquestionbtn', get_string('addquestionbtn', 'qtype_speechtesttask'));
	...
	protected function append_per_question_answers($i) {
		...
		for($j=1; $j <= $num_answers; $j++) {
			...
			$mform->addElement('radio', "correct_$i", '', get_string('correct', 'qtype_speechtesttask'), $j, array());
		}
	...
	protected function append_per_question_header($i, $num_questions) {
		...
		$this->_form->addElement('header', "qheader_$i", get_string('qheader', 'qtype_speechtesttask') . " $i " . $delbtn);
	}

	protected function append_per_answer_fields($i, $j, $num_answers) {
		$fields = array();
		$fields[] =& $this->_form->createElement('textarea', 'answer_'.$i.'_'.$j);
	...
	protected function get_num_questions() {
		...
		return 3; // default number of shown subquestions
	}
	...
	protected function get_num_answers($i) {
		...
		return 4; //default number of shown answerfields
	}
	...
	protected function data_preprocessing($question) {
		...
		// questions for instance -> take a look at qtype_speechtesttask::get_question_options()
	...
	public function get_answer_definition_html($num_current_subquestion) {
		// TestURL: http://localhost/moodle23/question/type/speechtesttask/ajaxiface.php?json_request_for_subquestion=1&answer_1_1=saf&correct_1=1
